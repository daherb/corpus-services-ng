...
public class AddCSVMetadataToComa extends Converter implements CorpusFunction {
    ...
    public Report check(CorpusData cd) throws SAXException, JexmaraldaException {
        ...
        try {
            ...
            stats.addException(getFunction(), pce, cd, "Unknown parsing error");
        } catch (IOException ioe) {
            stats.addException(getFunction(), ioe, cd, "Unknown file reading error");
        } catch (JDOMException ex) {
             stats.addException(getFunction(), ex, cd, "Unknown JDOM error");
        }
        return stats;
    }
    ...
    public Report function(CorpusData cd)
        ...
        for (String[] row : allElements) {
            ...
            stats.addNote(getFunction(), cd, Arrays.toString(row));
        }
        System.out.println(Arrays.toString(allElements.get(0)));
        stats.addNote(getFunction(), cd, Arrays.toString(allElements.get(0)));
        System.out.println(allElements.get(0)[0]);
        stats.addNote(getFunction(), cd, allElements.get(0)[0]);
        ...
        for (int i = 1; i < allElements.size(); i++) {
            for (int a = 1; a < allElements.get(i).length; a++) {
                if (IsSpeaker) {
                    ...
                    stats.addNote(getFunction(), cd, place);
                    ...
                    if (o != null) {
                        ...
                        stats.addNote(getFunction(), cd, Arrays.toString(desc.getAttributes().toArray()));
                ...
                if (!IsSpeaker) {
                    ...
                    XPath p = XPath.newInstance(place);
                    System.out.println(p.selectSingleNode(coma));
                    stats.addNote(getFunction(), cd, p.selectSingleNode(coma).toString());
                    Object o = p.selectSingleNode(coma);
                    if (o != null) {
                        Element desc = (Element) o;
                        //the new Key element that is inserted
        ...
        stats.addNote(getFunction(), cd, "The data in the csv file has been added into the coma.");
        return stats;
    }
