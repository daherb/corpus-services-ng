...
import de.uni_hamburg.corpora.functions.Converter;
    ...
    * and gives back a report how it worked
    ...
    public Report convertCOMA2MORPHEMEHIATISOTEI(CorpusData cd) throws ClassNotFoundException {
        ...
        try {
            ...
            for (Element communicationElement : communicationsList) {
                ...
                for (Element transcriptionElement : transcriptionsList) {
                    ...
                    if (finalDoc != null) {
                        ...
                        stats.addCorrect(getFunction(), cdc, "ISO TEI conversion of file was successful");
                    } else {
                        stats.addCritical(getFunction(), cdc, "ISO TEI conversion of file was not possible because of unknown error");
                    }
            ...
            stats.addException(getFunction(), ex, cd, "Unknown exception error");
        } catch (FSMException ex) {
            stats.addException(getFunction(), ex, cd, "Unknown finite state machine error");
        } catch (MalformedURLException ex) {
            stats.addException(getFunction(), ex, cd, "Unknown file URL reading error");
        } catch (JDOMException ex) {
            stats.addException(getFunction(), ex, cd, "Unknown file reading error");
        } catch (IOException ex) {
            stats.addException(getFunction(), ex, cd, "Unknown file reading error");
        } catch (TransformerException ex) {
            stats.addException(getFunction(), ex, cd, "XSL transformer error");
        } catch (ParserConfigurationException ex) {
            stats.addException(getFunction(), ex, cd, "Parser error");
        } catch (XPathExpressionException ex) {
            stats.addException(getFunction(), ex, cd, "XPath error");
        } catch (URISyntaxException ex) {
            stats.addException(getFunction(), ex, cd, "ComaPath URI error");
        } catch (JexmaraldaException ex) {
            stats.addException(getFunction(), ex, cd, "Jexmeaalda error");
        }
        return stats;
    }
    ...
    * and gives back a report if it worked
    ...
    public Report convertEXB2MORPHEMEHIATISOTEI(CorpusData cd,
        ...
        if (teiDoc != null) {
            ...
            System.out.println("document written.");
            stats.addCorrect(getFunction(), cd, "ISO TEI conversion of file was successful");
        } else {
            stats.addCritical(getFunction(), cd, "ISO TEI conversion of file was not possible because of unknown error");
        }
    ...
    public Collection<Class<? extends CorpusData>> getIsUsableFor() {
        Collection<Class<? extends CorpusData>> IsUsableFor = getIsUsableFor();
        IsUsableFor.add(de.uni_hamburg.corpora.data.content.EXMARaLDATranscriptionData.class);
