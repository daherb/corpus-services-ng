...
public class HandlePidRegistration extends Publisher implements CorpusFunction {
    ...
    public Report function(CorpusData cd)
        ...
        for (int x = 0; x < ElementNames.length; x++) {
            ...
            for (int i = 0; i < nodes.getLength(); i++) {
                ...
                if(oldURL.matches("^\\s*(https?://)?hdl\\.handle\\.net/.*$")){
                    stats.addWarning(getFunction(), cd, "URL is already a Handle PID: " + oldURL);
                } 
                // if URL is not already a Handle
                    ...
                    if(existingHandle != null && !existingHandle.equals("")){
                        ...
                        stats.addNote(getFunction(), cd, "Retrieved existing Handle PID for " + oldURL + ":\n" + newURL);
                    ...
                    /* for this url a new pid has to be registered */
                        ...
                        newURL = newURL.replaceAll("[\\s\\n]+", "");
                        stats.addNote(getFunction(), cd, "Registered new Handle PID for " + oldURL + ":\n" + newURL);
        ...
        try {
            ...
            stats.addFix(getFunction(), cd, "Handle PIDs were retrieved and file was updated.");
        } catch (UnsupportedEncodingException ex) {
            stats.addCritical(getFunction(), cd, "UnsupportedEncodingException: " + ex);
        } catch (XPathExpressionException ex) {
            stats.addCritical(getFunction(), cd, "XPathExpressionException: " + ex);
        } catch (TransformerException ex) {
            stats.addCritical(getFunction(), cd, "TransformerException: " + ex);
        }
    ...
    public Collection<Class<? extends CorpusData>> getIsUsableFor() {
        ...
        IsUsableFor.add(de.uni_hamburg.corpora.data.meta.CmdiData.class);
        return IsUsableFor;
    }
