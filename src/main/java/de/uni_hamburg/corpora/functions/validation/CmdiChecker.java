 ...
 * @author HZSK
 */
package de.uni_hamburg.corpora.functions.validation;
...
public class CmdiChecker extends Checker implements CorpusFunction {
    ...
    public Report function(CorpusData cd, Boolean fix)
        ...
        for (int i = 0; i < rps.getLength(); i++) {
            ...
            if (restype.getTextContent().equals("LandingPage")) {
                hasLandingPage = true;
                stats.addCorrect(getFunction(), cd, "Good resource type LandingPage");
            } else if (restype.getTextContent().equals("Resource")) {
                stats.addCorrect(getFunction(),  cd,
                ...
                stats.addWarning(getFunction(),  cd,
            ...
            if (!isUrlHandleOrHzsk(url)) {
                stats.addCritical(getFunction(), cd,
                        "Invalid URL for reesource proxy:"
                        + url +
                        "URLs should start with http://hdl.handle.net... or "
                        + "https://corpora.uni-hamburg.de/repository/...");
            } else {
                stats.addCorrect(getFunction(),  cd, "Good resource proxy URL " + url);
            }
        }
        if (!hasLandingPage) {
            stats.addCritical(getFunction(),  cd,  "Missing landing page");
        } else {
            stats.addCorrect(getFunction(),  cd,  "Good landing page found");
        }
        ...
        for (int i = 0; i < gis.getLength(); i++) {
            ...
            for (int j = 0; j < childs.getLength(); j++) {
                ...
                if (e.getTagName().equals("PID")) {
                    if (!isUrlHandleOrHzsk(e.getTextContent())) {
                        stats.addCritical(getFunction(),  cd, "Invalid URL for PID:"
                                + e.getTextContent() +
                                "URLs should start with "
                                + "http://hdl.handle.net... or "
                                + "https://corpora.uni-hamburg.de/repository/...");
                    } else {
                        stats.addCorrect(getFunction(), cd, "Good PID URL: "
                                + e.getTextContent());
                    }
                    pidFound = true;
                    ...
                    if (e.getAttribute("xml:lang").equals("en") || e.getAttribute("xml:lang").equals("eng")) {
                        englishDesc = true;
                        stats.addCorrect(getFunction(), cd, "English Description present");
                    }
                } else if (e.getTagName().equals("Title")) {
                    if (e.getAttribute("xml:lang").equals("en") || e.getAttribute("xml:lang").equals("eng")) {
                        englishTitle = true;
                        stats.addCorrect(getFunction(),  cd,  "English title present");
                    }
                } else if (e.getTagName().equals("LegalOwner")) {
                    legalOwner = true;
                    stats.addCorrect(getFunction(),  cd,  "LegalOwner present");
                } else {
            ...
            if (!englishTitle) {
                stats.addWarning(getFunction(), cd,  "English title missing from General Info "
                        + "(needed by FCS for example)");
            }
            if (!englishDesc) {
                stats.addWarning(getFunction(), cd,  "English Description missing from General Info "
                        + "(needed by FCS for example)");
            }
            if (!pidFound) {
                stats.addCritical(getFunction(),  cd,  "PID missing");
            }
        }
    ...
    private void checkCorpusInfo(Element ci, Report stats, CorpusData cd) {
        ...
        if (!corpusType) {
            stats.addCritical(getFunction(),  cd,  "Corpus type is needed for repo web pages");
        } else {
            stats.addCorrect(getFunction(),  cd,  "Corpus type included");
        }
        if (!genre) {
            stats.addCritical(getFunction(),  cd,  "Genre is needed for repo web pages");
        } else {
            stats.addCorrect(getFunction(),  cd,  "Genre included");
        }
        if (!modality) {
            stats.addCritical(getFunction(), cd,  "Modality is needed for repo web pages");
        } else {
            stats.addCorrect(getFunction(),  cd,  "modality included");
        }
        if (!timeCoverage) {
            stats.addWarning(getFunction(),  cd,  "time coverage is missing (recommended for VLO)");
        }
    }
    ...
    private void checkCoverage(Element coverage, Report stats, CorpusData cd) {
        ...
        for (int i = 0; i < timeCoverages.getLength(); i++) {
            ...
            if (tc.matches("[0-9]+/[0-9]+")) {
                stats.addCorrect(getFunction(),  cd,  "Good time coverage");
            } else {
                stats.addCritical(getFunction(),  cd,  "TimeCoverage should be YYYY/YYYY for VLO");
            }
        }
    ...
    private void checkSubjectLanguages(Element sls, Report stats, CorpusData cd) {
        ...
        for (int i = 0; i < langs.getLength(); i++) {
            ...
            if (!engFound) {
                stats.addCritical(getFunction(),  cd,  "Each subject language must have @xml:lang eng "
                        + "filled in");
            } else {
                stats.addCorrect(getFunction(),  cd,  "Goog language data");
            }
        }
    }
